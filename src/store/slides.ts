...
import { persistentDb } from '@/utils/database'
...
export const useSlidesStore = defineStore('slides', {
  ...
  actions: {
    ...
    async saveAsDefaultTemplate(password: string) {
      const { saveDefaultTemplate } = await import('@/api/template')
      // 序列化数据
      ...
      await saveDefaultTemplate(password, serializedData.slides, serializedData.theme)
    },

    async clearDefaultTemplate(password: string) {
      const { clearDefaultTemplate: clearTemplate } = await import('@/api/template')
      await clearTemplate(password)
    },

    async initSlides() {
      const { getDefaultTemplate } = await import('@/api/template')
      const defaultTemplate = await getDefaultTemplate()
      if (defaultTemplate) {
        this.slides = defaultTemplate.slides
        this.theme = defaultTemplate.theme
      }
    },
  },
})